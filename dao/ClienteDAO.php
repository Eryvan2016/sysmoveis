...
class ClienteDAO {
    ...
    function atualizar($obj) {
        require $_SERVER['DOCUMENT_ROOT'] . "/sysmoveis/bd/Conexao.php";
        try {
            $sql = "UPDATE cliente SET cpf=:cpf,nome=:nome,rua=:rua,bairro=:bairro,cidade=:cidade,numero=:numero,uf=:uf,pontoReferencia=:pontoReferencia, telefone=:telefone WHERE id = :id";
            $p_sql = $dbh->prepare($sql);
            $p_sql->bindValue(":id", $obj->getId());
            $p_sql->bindValue(":cpf", $obj->getCpf());
            $p_sql->bindValue(":nome", $obj->getNome());
